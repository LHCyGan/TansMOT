...
def distance_box_iou(boxes1, boxes2):
    ...
    if isinstance(boxes1, np.ndarray):
        lt = np.minimum(boxes1[:, None, :2], boxes2[:, :2])  # [N,M,2]
        rb = np.maximum(boxes1[:, None, 2:], boxes2[:, 2:])  # [N,M,2]
        wh = np.clip(rb - lt, 0, None)
        c_square = wh[None, 0]**2  + wh[None, 1]**2
        ...
        wh = (rb - lt).clamp(min=0)  # [N,M,2]
        c_square = wh[None, 0]**2 + wh[None, 1] ** 2  # [N,M]
    
    center1 = (boxes1[None, :2] + boxes1[None, 2:]) / 2
    center2 = (boxes2[None, :2] + boxes2[None, 2:]) / 2
    wh = center2 - center1
    d_square = wh[None, 0]**2 + wh[None, 1]**2  # [N,M]

    return iou - d_square / c_square
